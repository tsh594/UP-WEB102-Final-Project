  ...
  useEffect(() => {
    ...
    const fetchPostData = async () => {
      try {
        ...
        // In your fetchPostData function
  ...
  return (
    <div className="glass-panel post-page-container">
      ...
      {post.medical_references && (
        <div className="medical-references-section">
          <h3 className="references-heading">
            <FaStethoscope /> Medical References
          </h3>
          <div className="references-content">
            {post.medical_references.split('\n').map((line, index) => (
              <p key={index} className="reference-line">{line}</p>
            ))}
          </div>
        </div>
      )}
